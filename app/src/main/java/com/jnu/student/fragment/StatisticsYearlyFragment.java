...
public class StatisticsYearlyFragment extends Fragment {
    ...
    public StatisticsYearlyFragment() {}

    public static StatisticsYearlyFragment newInstance(String param1, String param2) {
        StatisticsYearlyFragment fragment = new StatisticsYearlyFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.fragment_statistics_yearly, container, false);

        lineCharts = new LineChart[3];
        lineCharts[0] = rootView.findViewById(R.id.chart_yearly_income);
        lineCharts[1] = rootView.findViewById(R.id.chart_yearly_pay);
        lineCharts[2] = rootView.findViewById(R.id.chart_yearly_surplus);
        updateChart();                 // 更新统计图
    ...
    private void updateValue(){
        ...
        for(int i = 0; i < eventList.size(); ++i){
            ...
            if(e.getTime().substring(0, 4).equals(formattedCurrentTime)){
                int eMonthOfYear = Integer.parseInt(e.getTime().substring(5, 7)) - 1;
                if(eMonthOfYear <= monthOfYear)
                    if(e.getEventValue() >= 0) data[0][eMonthOfYear] += e.getEventValue();
                    else data[1][eMonthOfYear] -= e.getEventValue();
            }
            else break;
        }
        ...
        for(int value = 0; value < monthOfYear + 1; ++value) {
            data[2][value] = data[0][value] - data[1][value];
            if(-1 == start && (0 != data[0][value] || 0 != data[1][value]))
                start = value;
        }
        float n = 0 == monthOfYear + 1 - start ? 1 : monthOfYear + 1 - start;     // n为计算均值时的除数
    ...
    private void updateChart(){
        ...
        for(int kind = 0; kind < 3; ++kind){
            ...
            if(0 == kind) description.setText("月均收入: " + average[kind]);
            else if(1 == kind) description.setText("月均支出: " + average[kind]);
            else description.setText("月均结余: " + average[kind]);
            ...
            lineCharts[kind].moveViewToX(monthOfYear - 8.2f);                  // 将最初视图移到最右端
            ...
            xAxis.setAxisMaximum(11.2f);                              // +0.2f使最后一个标签可以显示
