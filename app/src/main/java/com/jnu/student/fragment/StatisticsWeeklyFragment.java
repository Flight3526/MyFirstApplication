...
public class StatisticsWeeklyFragment extends Fragment {
    ...
    public StatisticsWeeklyFragment() {}

    public static StatisticsWeeklyFragment newInstance(String param1, String param2) {
        StatisticsWeeklyFragment fragment = new StatisticsWeeklyFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.fragment_statistics_weekly, container, false);

        lineCharts = new LineChart[3];
        lineCharts[0] = rootView.findViewById(R.id.chart_weekly_income);
        lineCharts[1] = rootView.findViewById(R.id.chart_weekly_pay);
        lineCharts[2] = rootView.findViewById(R.id.chart_weekly_surplus);
        updateChart();                 // 更新统计图
    ...
    private void updateValue(){
        ...
        dayOfWeek = calendar.get(Calendar.DAY_OF_WEEK) - 1;   // 令周日为0
        int weekOfYear = calendar.get(Calendar.WEEK_OF_YEAR);
        int year = calendar.get(Calendar.YEAR);
        data = new int[3][dayOfWeek + 1];
        average = new float[3];
        ...
        Date eDate;
        int eWeekOfYear, eDayOfWeek, eYear;
        for(int i = 0; i < eventList.size(); ++i){
            EventItem e = eventList.get(i);
            try{
                eDate = sdf.parse(e.getTime().substring(0,10));
            } catch(Exception exc){
                exc.printStackTrace(); continue;
            }
            calendar.setTime(eDate);

            eWeekOfYear = calendar.get(Calendar.WEEK_OF_YEAR);
            eDayOfWeek = calendar.get(Calendar.DAY_OF_WEEK) - 1;
            eYear = calendar.get(Calendar.YEAR);
            if(year == eYear && weekOfYear == eWeekOfYear){
                if(eDayOfWeek <= dayOfWeek)
                    if(e.getEventValue() >= 0) data[0][eDayOfWeek] += e.getEventValue();
                    else data[1][eDayOfWeek] -= e.getEventValue();
            }
            else break;
        }
        ...
        for(int value = 0; value < dayOfWeek + 1; ++value) {
            data[2][value] = data[0][value] - data[1][value];
            if(-1 == start && (0 != data[0][value] || 0 != data[1][value]))
                start = value;
        }
        float n = 0 == dayOfWeek + 1 - start ? 1 : dayOfWeek + 1 - start;     // n为计算均值时的除数
    ...
    private void updateChart(){
        ...
        for(int kind = 0; kind < 3; ++kind){
            ...
            lineCharts[kind].setVisibleXRangeMaximum(7.2f);                           // 设置可见的X轴范围的最大值
            lineCharts[kind].setVisibleXRangeMinimum(0f);                             // 设置可见的X轴范围的最小值
            lineCharts[kind].moveViewToX(0f);
            ...
            xAxis.setAxisMaximum(6.2f);                               // +0.2f使最后一个标签可以显示
            xAxis.setValueFormatter(new ValueFormatter() {            // 转换轴标签
                public String getFormattedValue(float value) {
                return "周" + "日一二三四五六".charAt((int) value);
                }
            });
